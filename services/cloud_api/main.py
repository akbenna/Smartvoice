...
Endpoints:
  POST /api/v1/consult/process  - Full pipeline (audio → results)
...
async def process_consult(
    ...
    stt_provider: str | None = Form(None, description="STT provider override"),
    llm_provider: str | None = Form(None, description="LLM provider override"),
    practice_id: str | None = Form(None, description="Praktijk identifier"),
    practitioner_name: str | None = Form(None, description="Naam behandelaar"),
    _api_key: str = Depends(verify_api_key),
):
    """
    Process a consultation audio recording through the full pipeline.

    Returns SOEP note, decisief regel, and red flag detection.
    """
...
async def list_providers(_api_key: str = Depends(verify_api_key)):
    """List available STT and LLM providers with their configuration status."""
    cfg = get_config()
    return {
        "stt": {
